...
def load_and_process_common_accent_dataset(args):
    ...
    common_accent["train"] = load_dataset(
        args.dataset_name, split="train", trust_remote_code=True
    )
    common_accent["test"] = load_dataset(
        args.dataset_name, split="test", trust_remote_code=True
    )
    ...
    tokenizer = WhisperTokenizer.from_pretrained(
        args.model_id, language=args.language, task=args.task
    )
    ...
    common_accent = common_accent.map(
        prepare_dataset, remove_columns=common_accent.column_names["train"], num_proc=1
    )
    return {
...
def load_and_process_dataset(args):
    ...
    common_voice["test"] = load_dataset(
        args.dataset_name, args.language_id, split="test", trust_remote_code=True
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--language", type=str, default="Hindi", help="Language for the Whisper model."
    )
    parser.add_argument(
        "--language_id", type=str, default="hi", help="Language for the Whisper model."
    )
    parser.add_argument(
        "--task", type=str, default="transcribe", help="Task for the Whisper model."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--learning_rate", type=float, default=4e-6, help="Learning rate for training."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max_steps", type=int, default=5000, help="Maximum number of training steps."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of evaluations with no improvement to stop training.",
    )
    parser.add_argument(
        "--early_stopping_threshold",
        type=float,
        default=0.001,
        help="Minimum change to be considered an improvement for early stopping.",
    )
