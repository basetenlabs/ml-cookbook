...
By default, the agent runs two experiments per iteration. Setting `PARALLEL_JOBS` higher (up to 5) tells the agent to generate more hyperparameter variants per iteration, submit them all to Baseten simultaneously, and keep only the best result. This increases throughput roughly proportionally at the cost of more GPU-hours per iteration.
...
Find the job ID of your best experiment in `results.tsv` or the Baseten UI, then deploy it:

```bash
truss train deploy_checkpoints --job-id <job_id>
```

Baseten constructs a deployment configuration from the checkpoint, provisions an inference server, and gives you an endpoint you can call immediately. If you'd prefer to inspect or customize the configuration before it goes live, the `--dry-run` flag generates the deployment config on disk without pushing it. You can also deploy directly from the Baseten UI by selecting "Deploy" on any completed job's checkpoint dropdown.

For more advanced workflows, including deploying custom model architectures or checkpoints sharded across multiple nodes, see the [serving trained models](https://docs.baseten.co/training/deployment) documentation.
