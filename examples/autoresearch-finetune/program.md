...
Poll the job status using `truss train logs`:

```bash
truss train logs --job-id <job_id> --remote $REMOTE --non-interactive
```

Re-run this command periodically until the logs show training has completed (look for the structured results block at the end) or the job has failed. You can also check job status via the Baseten UI link printed by the push command.
...
### Sequential mode (PARALLEL_JOBS=1, default)
...
7. Poll `truss train logs --job-id <job_id> --remote $REMOTE --non-interactive` until the job completes or fails.
...
4. Poll `truss train logs` for all N jobs until they complete.
...
**Caching**: The first job in a new project is slower because it downloads model weights and the dataset. Subsequent jobs reuse the Baseten project cache (`BT_RW_CACHE_DIR`), so startup is much faster.
