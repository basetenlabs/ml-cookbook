#!/usr/bin/env python3
"""s3-experiment-dashboard server (multi-run variant of the rollout-dashboard
renderer).

Serves a multi-run experiment store: EXPERIMENTS_ROOT/experiments.json is the
cross-run index (compare page), and each RUNS_ROOT/<run_id>/ dir with its own
run.json gets the full single-run dashboard at /run/<run_id>/.
...
EXPERIMENTS_PATH = EXPERIMENTS_ROOT / "experiments.json"

# Static file extensions we serve from EXPERIMENTS_ROOT
STATIC = {
...
def load_experiments():
    if not EXPERIMENTS_PATH.exists():
        return {"runs": [], "_error": f"experiments.json not found at {EXPERIMENTS_PATH}"}
    try:
        return json.loads(EXPERIMENTS_PATH.read_text())
    except json.JSONDecodeError as e:
        return {"runs": [], "_error": f"experiments.json failed to parse: {e}"}


def resolve_run_dir(run_id: str):
    """Map a run_id from the URL to a dir under RUNS_ROOT. None if invalid."""
    if not run_id or "/" in run_id or "\\" in run_id or run_id in (".", ".."):
        return None
    d = (RUNS_ROOT / run_id).resolve()
    if d.parent != RUNS_ROOT.resolve() or not d.is_dir():
        return None
    return d
...
def load_source_records(source: dict, run_dir: Path):
    """Load all records for one rollout source. Returns list[dict]."""
    paths = resolve_paths(source.get("glob") or source["file"], run_dir)
    all_recs = []
...
def load_metrics_records(metrics_cfg, run_dir: Path):
    if not metrics_cfg: return []
    paths = resolve_paths(metrics_cfg["file"], run_dir)
    recs = []
    for p in paths:
...
def find_rollout(source, rollout_id, run_dir: Path):
    """Return the single record matching rollout_id. None if not found."""
    recs = load_source_records(source, run_dir)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # Browsers request this automatically; the original server 404'd it,
        # which shows up as a console error. Answer with an empty 204 instead.
        if path == "/favicon.ico":
        ...
        # /experiments → the cross-run index (compare page data)
        if path == "/experiments":
            return self._send(200, load_experiments())

        # /run/<run_id>[/...] → the single-run dashboard, scoped to that run
        if path.startswith("/run/"):
            parts = path.split("/", 3)          # ['', 'run', '<run_id>', 'rest']
            run_id = urllib.parse.unquote(parts[2]) if len(parts) > 2 else ""
            run_dir = resolve_run_dir(run_id)
            if run_dir is None:
                return self._send(404, {"error": f"unknown run {run_id!r}"})
            sub = parts[3] if len(parts) > 3 else None

            # /run/<id> (no trailing slash) → redirect so relative fetches work
            if sub is None:
            ...
            # /run/<id>/data → full RUN payload (config + materialized records)
            if sub == "data":
                cfg = load_config(run_dir)
                self._materialize_records(cfg, run_dir)
                return self._send(200, cfg)

            # /run/<id>/detail/<sourceIdx>/<rolloutId>
            if sub.startswith("detail/"):
                dparts = sub.split("/", 2)
                if len(dparts) >= 3:
                    try:
                        source_idx = int(dparts[1])
                    except ValueError:
                        return self._send(400, {"error": "bad source index"})
                    rollout_id = urllib.parse.unquote(dparts[2])
                    ...
                    record = find_rollout(source, rollout_id, run_dir)
                    if record is None:
        ...
        # Static files. The compare page is the landing page.
        if path in ("/", "/compare"):
            return self._serve_static("compare.html")
        if path in ("/index.html", "/compare.html", "/style.css", "/renderers.js",
                    "/detail.html", "/chart.umd.min.js"):
    ...
    def _materialize_records(self, cfg, run_dir: Path):
        """Mutate cfg to add _records arrays for each source + metrics."""
        for src in cfg.get("rollout_sources", []):
            try:
                src["_records"] = load_source_records(src, run_dir)
            except Exception as e:
                src["_records"] = []
                src["_error"] = str(e)
        if cfg.get("metrics"):
            try:
                cfg["metrics"]["_records"] = load_metrics_records(cfg["metrics"], run_dir)
            except Exception as e:
...
def print_summary(experiments, port):
    print(f"\ns3-experiment-dashboard → http://localhost:{port}\n")
    if experiments.get("_error"):
        print(f"  ⚠ {experiments['_error']}")
        print("  run `./sync.sh && python3 build_manifests.py` first.")
        return
    runs = experiments.get("runs", [])
    if experiments.get("s3_prefix"):
        print(f"  s3 prefix: {experiments['s3_prefix']}")
    print(f"  runs ({len(runs)}):")
    for r in runs:
        s = r.get("summary") or {}
        print(f"    · {r['run_id']} [{r.get('source', '?')}] — "
              f"step {s.get('last_step', '?')} · "
              f"best eval_loss {s.get('best_eval_loss', '?')} → /run/{r['run_id']}/")
    print("\n  compare view: /   ·   re-sync: ./sync.sh && python3 build_manifests.py")
    print()
...
def main():
    ...
    print_summary(experiments, port)
